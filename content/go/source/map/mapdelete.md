...
仔细观察删除逻辑，你会发现 `delete` 操作仅仅是清空了 key 和 value 的内存，并重置了 `tophash` 状态。**它并没有减少 bucket 的数量，也没有缩小 map 的底层数组大小。**

这意味着，如果你有一个曾经存储了海量数据（触发了多次扩容）的 map，即使你后来使用 `delete` 删除了其中 99% 的元素，这个 map 依然会占用与巅峰时期一样多的 bucket 内存。这些空闲的 bucket 不会被垃圾回收器（GC）回收。

### 何时该警惕？

在以下场景中，这种“只增不减”的机制会导致严重的内存问题：
- **长生命周期的 map**：例如全局缓存、会话管理等。
- **数据量剧烈波动的 map**：周期性地写入大量数据，随后又大量删除。

### 如何应对？

如果你面临上述场景，不要依赖 `delete` 来释放内存。推荐的做法是：
定期创建一个新的、容量合适的小 map，将旧 map 中仍然有效的少量数据拷贝过去，然后将旧 map 的引用置为 `nil`。这样，旧 map 及其庞大的 bucket 数组就能被 GC 彻底回收。