...
:::: [!question]- 问：返回 nil 了我还怎么获取到对象？

- 如果提供了 New 函数，池会自动帮你处理空则创建的逻辑

```go fold="New()"
var pool = sync.Pool{
    New: func() any { return new(Object) },
}

func main() {
    // 永远不会返回 nil，因为 New 会被调用
    obj := pool.Get().(*Object) 
    defer pool.Put(obj)
    // ... 使用 obj
}
...
- 如果没有提供 New 函数，则需要调用者自己负责兜底创建

```go fold="调用者手动创建对象"
var pool = sync.Pool{} // 没有 New
...
::: [!tip] 设计更灵活
当对象比较简单时，定义好 New 函数可以方便后续调用者直接获取。当遇到比较复杂的场景和对象时，不提供 New 函数可以交由调用者按需创建。
:::
...
1.  如果你有 `n` 个逻辑处理器 (P)，你可以==并行==运行最多 `n` 个 Goroutine，只要你至少有 `n` 个机器线程 (M) 可用。
